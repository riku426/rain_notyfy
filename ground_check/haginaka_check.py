...
class Get_Hagi_Ground():
    ...
    def get_ground(self):
        ...
        time.sleep(1)
        #萩中グラウンドをクリック
        self.driver.find_element_by_xpath("/html/body/form[2]/div/table/tbody/tr[4]/td/table[1]/tbody/tr[4]/td[2]/table/tbody/tr[2]/td[1]/input").click()
        time.sleep(1)
        #本日のグラウンド名を取得
    ...
    def rain_check(self):
        ...
        for i in range(len(self.text)):
            if self.text[i] == "コート不良":
                ...
                pass
        with open("csv/state_of_hagi.csv", "a", newline="") as f:
            writer = csv.writer(f)
            writer.writerow(self.text)

        alltxt = pd.read_csv("csv/state_of_hagi.csv", "r", encoding="cp932").values.tolist()
